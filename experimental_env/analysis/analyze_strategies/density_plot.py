...
class DensityPlot(AnalysisStrategy):
    ...
    def save_analysis(self):
        plt.set_yscale = self._y_scale
        plt.legend()
        plt.savefig(self._out_dir)

    def analyze_method(self, result: ExtendedResultDescription, method: str):
        X = np.linspace(-10, 10, 200)

        plt.hist(result.samples, color="lightsteelblue")
        plt.plot(X, [result._base_mixture.pdf(x) for x in X], color="red", label="base")
        plt.plot(X, [next(result.steps).result.content.pdf(x) for x in X], color="green", label="estimated")
        self.save_analysis()

    def compare_methods(self, result_1: ExtendedResultDescription, result_2: ExtendedResultDescription, method_1: str, method_2: str):
        X = np.linspace(-10, 10, 200)

        plt.hist(result_1.samples, color="lightsteelblue")
        plt.plot(X, [result_1._base_mixture.pdf(x) for x in X], color="red", label="base")
        plt.plot(X, [next(result_1.steps).result.content.pdf(x) for x in X], color="green", label=method_1)
        plt.plot(X, [next(result_2.steps).result.content.pdf(x) for x in X], color="purple", label=method_2)
        self.save_analysis()