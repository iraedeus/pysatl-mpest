...
class Cauchy(AModelWithGenerator, AModelDifferentiable):
    ...
    This class implements the Cauchy probability distribution (also known as Lorentz
    distribution). It inherits functionality for sample generation from AModelWithGenerator
    and for computing derivatives from AModelDifferentiable.

    The Cauchy distribution is parameterized by:
      - External parameters: location ($x_0$) and scale ($\\gamma > 0$)
      - Internal model parameters: location ($x_0$) and log-scale ($\\sigma = \\log(\\gamma)$)
    ...
    def params_convert_to_model(self, params: Params) -> Params:
        ...
        Transforms the external parameterization [$x_0$, $\gamma$] to internal model
        parameters [$x_0$, $\sigma$] where $\sigma = \log(\gamma)$.

        Args:
            params (Params): External parameters [$x_0$, $\gamma$] where $x_0$ is the location
                parameter and gamma is the scale parameter.

        Returns:
            Params: Internal model parameters [$x_0$, $\log(\gamma)$].
        """
    ...
    def params_convert_from_model(self, params: Params) -> Params:
        ...
        Transforms the internal model parameters [$x_0$, $sigma$] to external
        parameterization [$x_0$, $\gamma$] where $\gamma = \e^{\sigma}$.

        Args:
            params (Params): Internal model parameters [$x_0$, $\sigma$] where $x_0$ is the location
                parameter and sigma is the log-scale parameter.

        Returns:
            Params: External parameters [$x_0$, $e^{\sigma}$].
        """
    ...
    def generate(self, params: Params, size: int = 1, normalized: bool = True) -> Samples:
        ...
        Args:
            params (Params): Internal model parameters [$x_0$, $\sigma$] when normalized=True,
                or external parameters [$x_0$, $\gamma$] when normalized=False.
    ...
    def pdf(self, x: float, params: Params) -> float:
        """Calculates the probability density function (PDF) value at $x$.
    ...
    def lpdf(self, x: float, params: Params) -> float:
        ...
        Returns:
            float: The logarithm of the PDF value at point $x$.
        """
    ...
    def ldx0(self, x: float, params: Params) -> float:
        """Calculates the partial derivative of the log-PDF with respect to $x_0$.

        Computes the derivative:
        $$\frac{\\partial\\ln(f(x; x_0, \\sigma))}{\\partial x_0} = \frac{2z}{(z^2 + 1) \\cdot e^{\\sigma}}$$
        where $z = \\dfrac{x - x_0}{e^{\\sigma}}$
        ...
        Returns:
            float: The partial derivative of the log-PDF with respect to $x_0$ at point x.
        """
    ...
    def ldsigma(self, x: float, params: Params) -> float:
        """Calculates the partial derivative of the log-PDF with respect to sigma.
    ...
    def ld_params(self, x: float, params: Params) -> np.ndarray:
        ...
        Returns:
            ...
            respect to each parameter at point x.
        """
